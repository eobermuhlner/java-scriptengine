 ...
 *
 * Slightly modified from the original implementation in jdk.jshell.MemoryFileManager
 *
 * @author Robert Field
 */
class MemoryFileManager implements JavaFileManager {
    ...
    public ClassLoader getClassLoader(JavaFileManager.Location location) {
        ClassLoader classLoader = stdFileManager.getClassLoader(location);
        if (location== CLASS_OUTPUT) {
            Map<String, byte[]> mapClassBytes = new HashMap<>();
            for (OutputMemoryJavaFileObject outputMemoryJavaFileObject : generatedClasses()) {
                mapClassBytes.put(outputMemoryJavaFileObject.getName(), outputMemoryJavaFileObject.getBytes());
            }
            return new MemoryClassLoader(mapClassBytes, classLoader);
        }
        return classLoader;
    }
